...
myApp.controller('cau1_controller', function ($scope, $state, $http, $location) {
    var vm = this;
    ...
    this.loadData = function (page_number) {
        ...
        $http.get('model/select-cau1.php?page=' + page_number + '&search_input=' + search_input).then(function (response) {
            vm.sinhvien_list = response.data.sinhvien_data;
            $scope.total_row = 2 * +response.data.total;
            // alert($scope.total_row);
        });
    };
